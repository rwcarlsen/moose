...
AuxiliarySystem::computeElementalVars(ExecFlagType type)
{
  ...
  std::string compute_aux_tag = "computeEverything(" + Moose::stringify(type) + ")";
  Moose::perf_log.push(compute_aux_tag, "Execution");
  ...
  PARALLEL_TRY
  {
    ...
    const MooseObjectWarehouse<SideUserObject> & side = _fe_problem._side_user_objects[Moose::ALL][type];
    const MooseObjectWarehouse<InternalSideUserObject> & internal_side = _fe_problem._internal_side_user_objects[Moose::ALL][type];

    ComputeEverythingThread ev(_fe_problem, _fe_problem.getNonlinearSystemBase(), elemental, pre_user_obj, post_user_obj, side, internal_side);
