...
#define LOOP5_DOF_QP_SECOND_OR_SECOND_OLD_OR_SECOND_OLDER_OR_SECOND_PREVIOUS_NL                    \
  d2phi_local = &(*_second_phi)[i][qp];                                                            \
                                                                                                   \
  if (_need_second)                                                                                \
  {                                                                                                \
...
MooseVariable::computeElemValues()
{
  ...
  else if (is_transient && second_cond)
  {
    for (unsigned int i = 0; i < num_dofs; i++)
    {
      LOOP1_DOF_MAIN
