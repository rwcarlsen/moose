...
void
ComputeEverythingThread::postElement(const Elem * elem)
{
}
...
ComputeEverythingThread::onBoundary(const Elem * elem, unsigned int side, BoundaryID bnd_id)
{
  ...
  // Something like this should eventually supplant ComputeElemAuxBcsThread
  //const auto & boundary_kernels = _aux_kernels.getActiveBoundaryObjects(_tid);
  //const auto iter = boundary_kernels.find(bnd_id);
...
void
ComputeEverythingThread::onInternalSide(const Elem * elem, unsigned int side)
{
