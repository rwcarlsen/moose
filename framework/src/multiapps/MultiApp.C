...
LocalAppConfig
rankConfigDeprecated(unsigned int rank,
                     unsigned int nprocs,
                     unsigned int napps,
                     ...
                     unsigned int max_app_procs)
{
  ...
  if (napps >= nprocs)
  {
    ...
    auto n_local_apps = napps / nprocs;

    unsigned int jobs_left = napps - (n_local_apps * nprocs);

    if (jobs_left != 0)
    {
      // Spread the remaining jobs out over the first set of processors
      if (rank < jobs_left) // (these are the "jobs_left_pids" ie the pids that are
                            // snatching up extra jobs)
      {
        n_local_apps += 1;
        app_index = n_local_apps * rank;
      }
      else
      {
        unsigned int num_apps_in_jobs_left_pids = (n_local_apps + 1) * jobs_left;
        unsigned int distance_to_jobs_left_pids = rank - jobs_left;

        app_index = num_apps_in_jobs_left_pids + (n_local_apps * distance_to_jobs_left_pids);
      }
    }
    else
      app_index = n_local_apps * rank;
  ...
  for (unsigned int i = 0; i < nprocs; ++i)
  {
    ...
    if (block_ind == procs_per_app)
    {
      ...
      if (my_app == (napps - extra_proc_apps))
        ++procs_per_app;
    }
  }
  return {n_local_apps, app_index};
